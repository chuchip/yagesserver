 ...
 */
  ...
  public static synchronized String FormatChar(String numero,String formato,boolean ceroIsNull, boolean elimPunto)
  {
      ...
      n = buscaletra(formato, '.', 0);
      if (n >= 0)
      {
          if (n == 0)
              fent = "#";
          else
              fent = formato.substring(0, n); // Parte entera del formato

          fdec = formato.substring(n + 1); // Parte Decimal del formato
      } else
          fent = formato; // Parte entera del formato
  ...
  public static String getFechaVer(java.util.Date fecha)
   {
    ...
    public static java.sql.Date getSqlDate(String fecha,String formfec) throws java.text.ParseException
	  {
	     if (fecha==null)
	       return null;
	     ...
	     try
	     {
	         return new java.sql.Date(sd.parse(fecha).getTime());
    ...
    * @param fecha Objeto Date con la fecha
    *
    * @return Fecha en formato String segun el Locale Actual
    */
...
//          Logger.getLogger(Formatear.class.getName()).log(Level.SEVERE, null, ex);
   ...
   */
    ...
    */
    ...
    public static java.sql.Date sumaDiasDate(java.sql.Date fecha,int numDias)
    {
    	return sumaDiasDate(fecha,numDias);
    }
